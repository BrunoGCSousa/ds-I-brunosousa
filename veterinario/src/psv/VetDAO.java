...
public class VetDAO {
    ...
    public String inserir(VetBean vet) {
        String sql = "insert into paciente(id, nome,idade,raca,porte,nascimento,vacinado,responsavel,telefone,cpf,endereco,bairro,numero,sintomas)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public String alterar(VetBean vet){
        String sql = "update paciente set nome = ?, idade = ?, raca = ?, porte = ?, nascimento = ?, vacinado = ?,responsavel = ?, telefone = ?, cpf = ?, endereco = ?, bairro = ?, numero = ?, sintomas = ?";
        sql += " where id = ?";
